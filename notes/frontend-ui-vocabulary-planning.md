...
>
> **2026-05-28 結構整理**：刪除空的「開場」section、移除「文章主結構（草擬）」H2 wrapper、把章節從 `###` 升到 `##`、章節從六章重整為五章 + 結語、6 個待思考問題嵌入正文對應位置。
...
> 🤔 **過幾天回來思考的問題 Q6（MoT 英文版 sanity check）**
>
> 不為英文版做 framing 妥協，但動筆中文版時腦中跑一遍：
> - 中文版用「我做 QA 那麼多年」這種第一人稱口吻——英文版會自然變樣，這沒問題
> - 但「**PDT 反向應用**」這個 framing 要不要在中文版就用**更通用的詞**（例如「**AI 暴露我沒問過的問題**」）？這樣英文版改寫成「reverse exploratory testing / AI as a vocabulary mirror」會無痛
>
> → 確認中文版的核心論述在英文版能 translate，不需要重寫。
...
> 🤔 **過幾天回來思考的問題 Q2（讓「跳第三欄」真的 work）**
>
> 表格散落在 7 個 sub-section 裡——讀者真的會跳嗎？
>
> **兩個選擇**：
> - **A. 加 anchor 連結列表**（放在 hook 段下方）：「Loading states / User feedback / Form states / Navigation / Information density / 動畫 / 存取控制——直接跳」
> - **B. 每節標題前加 emoji 標記**讓 scroll 時可以快速辨識（例如 ⏳ Loading / 💬 Feedback / ...）
>
> **推薦 A**——anchor 連結對「跳第三欄」更直接、SEO 也加分。

---

> 🤔 **過幾天回來思考的問題 Q3（章節間 transition prose 寫多細？）**
>
> **兩個選擇**：
> - **薄版**：每節之間 1 句話承接（「Loading 狀態看完，接下來看更貼近『使用者感受』的反饋元件」）
> - **厚版**：每節開頭一段 introduction，說明「為什麼這類詞彙的測試特別容易漏」
>
> 厚版會把文章從 4000 字推到 5500-6000 字。**推薦薄版**——詞彙文章本來就是讀者快速 scan，厚版會稀釋每個 entry 的能見度。
...
## 二、31 個前端 UI 詞彙、7 小類：你的最小生存 vocab

> 🤔 **過幾天回來思考的問題 Q1（最高 ROI 的內容投資）**：哪 3-5 個詞彙要擴成 callout box？
>
> 這是這篇從「**字典**」升級成「**有溫度的測試手冊**」的關鍵。**沒有 callout 這篇就像高品質維基百科**；有 3-5 個就變成有故事的手冊，讀者一秒記住、可信度 +30%。
>
> **候選**（挑 3-5 個你真的測過、寫成 100-200 字的真實案例）：
> - Modal / focus trap（鍵盤陷阱經典）
> - Optimistic UI rollback（網路慢時 rollback 失敗）
> - Inline validation 過早觸發（onChange vs onBlur 那種）
> - Toast 堆疊行為（多個同時跳出時）
> - CTA loading 不一致
>
> **格式範例**：
> > **真實案例（Modal / focus trap）**：某次我發現一個對話框，鍵盤 Tab 鍵會把焦點跑到背景的元素上——使用者用螢幕閱讀器或者鍵盤操作會誤觸背景，整個對話框等於白做了。這個 bug 自動化 100% 抓不到，要 QA 手動 Tab 過去才會看到。

### 2.1 載入狀態（Loading states）
...
| **Infinite scroll** | 滾到底自動載下一頁（社群動態、商品列表常見） | **載入時機**（離底多遠開始 fetch）、**重複載入**（同一筆資料出現兩次）、**到底邊界**（沒下一頁時的提示）、**回滾位置保留**（從詳細頁返回時不掉到頂） |

### 2.2 用戶反饋（User feedback）
...
| **CTA（Call to Action）** | 畫面上希望使用者點的主要動作（「立即註冊」「升級」「開始試用」）。**不限於按鈕**——可能是 banner、卡片、整塊區域。本質是邀請使用者執行漏斗下一步動作（通常是掏錢） | **視覺優先級明確**（主 CTA / 次 CTA 對比清楚）、**文字具體**（「立即註冊」優於「OK」）、loading / disabled 狀態完整、整個路徑是否真的引導使用者完成那個動作 |
...
### 2.7 存取控制 / 商業化（Access control & monetization）
...
## 三、進階：accessibility 一節

> 🤔 **過幾天回來思考的問題 Q4（accessibility 段怎麼處理）**：
>
> **三個選擇**：
> - **A. 砍掉**，當下一篇引子（5/14 的原規劃）
> - **B. 補成完整 entry**（4 個 a11y 詞彙也做三欄表格）
> - **C. 留簡短版 + 明標「下一篇深寫」**——告訴讀者「**這篇先點到，下一篇會深寫 a11y**」
>
> **推薦 C**——把缺陷變成 hook、建立讀者期待。

這些是 QA 最容易忽略、但對企業客戶 / 政府客戶最常被查的：
...
→ accessibility 是**下一篇要深寫的世界**——很多 QA 完全沒見過這層。
...
> 🤔 **過幾天回來思考的問題 Q5（延伸閱讀要不要加 / 加哪幾條）**：
>
> 文末加 3-4 條連結 → 讀者讀完願意繼續探索 → 站內滯留時間 + 文章互引強度都 +。
>
> **建議的 3 條**（都跟本篇主題強相關）：
> - [問題驅動測試（PDT）](/post/problem-driven-testing-intro/)——招牌主張，本篇是 PDT 反向應用
> - [你以為 AI 在做稱職的 QA，其實它在當你的 Yes Man](/post/ai-as-yes-man-rd-pm-trust-calibration/)——AI 測試的反向應用
> - [「煩」是 QA 的診斷工具](/post/boredom-is-a-signal/)——自動化判斷的思考工具
>
> **推薦都加**。
...
>
> 下次 AI 給你的測試案例裡有個詞讓你停了三秒——那不是 AI 太炫，是 AI 替你照亮一個你還沒學的角落。**接住那個三秒，就接住了一個成長機會**。
...
## 寫作前還沒解決的（**Q1-Q6 已嵌入正文對應位置，剩這些長期問題**）

- [ ] **截圖 / 圖示要不要做**：詞彙型文章配圖會大幅加分。可以用 CodePen 或現有 UI library 截圖
- [ ] **觸發條件**：不急，但 SEO 入門線有需要。**可以較早動筆**（沒有同事/主管指紋顧慮）
- [ ] **可能的姊妹篇**：「**QA 該認識的 accessibility**」「**QA 該認識的 responsive design**」——如果這篇反應好，可以做系列

---

## 一句話定位

> QA 看到 AI 給的 `skeleton loading` 卡住三秒——那一刻不是 AI 不對，是**自己從沒問過自己這個詞彙缺口**。這篇幫你補上基本詞彙。
