...
- [analogy-series-proposals.md](skeletons-active/analogy-series-proposals.md) — PDT 類比系列（醫生 / 記者 / 偵探 / 讀者）
...
- [pdt-coach-session-2026-07-03-insights.md](coach-sessions/2026-07-03-insights.md) — **Org-Signal Coach（6/16-7/03，旺季+端午較稀疏）**：**職涯方向的清明**（不是待在學不到東西的地方，是待在餵養你真正方向＝組織/協作/判斷的地方；SRE/SDET 是 AI 最會侵蝕的路）+ **難量化≠沒價值**（轉成「具體事件/決策/結果」、辨認無聲的階段性成果）+ **宏大敘事三競爭**（技術/權力/「把複雜系統的混亂變清楚的人」＝最有生命力那個）+ **冒牌貨感=用量不到自己的尺**（同「忙=價值」病根、用外顯標準量難量化的能力）+ **一個轉折：工作 Coach 階段性任務近完成、更重要的線在工作以外**。新孕育 3 角度（〈PM=客戶關係管理〉⭐⭐⭐ 無指紋最好動 + 〈難量化能力變看得見〉⭐⭐⭐ + 〈冒牌貨感=量錯的尺〉⭐⭐⭐⭐ 最強但最私人、留狀態好再寫）。**全偏線六/九（超載），優先挑無指紋的 PM=CRM**
...
- [pdt-vs-qbt-positioning.md](skeletons-done/pdt-vs-qbt-positioning.md) — **2026-06-01 戰略檔**：QBT 不是業界 established term（WebSearch + Preely 驗證後）；Preely 的 QBT 在 UX 領域、不撞義；PDT 對內 / QBT 對外的雙層架構討論。**rebrand 成本評估：PDT 投資 47 檔案不划算**
- [rethinking-regression-sequel-idea.md](skeletons-active/rethinking-regression-sequel-idea.md) — 〈4 個刪測試問題加第 5 個〉的續篇 idea（5/15）
...
- [cross-app-locator-strategy.md](skeletons-done/cross-app-locator-strategy.md) — 跨 APP 測試框架的元素定位策略（漸進式三階段：fallback chain → LocatorStrategy → per-APP class）。**含公司指紋**，blog 化需洗（5/28）→ ⭐⭐⭐⭐ 長尾力，建議落線三、跟 pom-refactor 形成 POM 雙篇
...
> - **2026-06-01：發現 PDT vs QBT 命名戰略議題**——驗證後 QBT 不是業界 established term、Preely 的 QBT 在 UX 領域不撞義。詳見 [pdt-vs-qbt-positioning.md](skeletons-done/pdt-vs-qbt-positioning.md)
...
### 🆕 冒煙框架實戰系列（2026-07-01 從真實 smoke 專案長出，7 篇骨架）

> 來源＝`AutoTesting/Author/smoke_script/docs/` 的真實文件（單兵 7-9 App 冒煙框架、跑在 CI）。**這是「有真部署撐、Q&A 打不倒」的料**——駁「我沒真的導入的東西可寫」。**全部指紋極重（公司/app/package/同事名），blog 化務必重度去識別。** 骨架見 skeletons-active/。

第一梯（最原創 / 最 on-brand）：
- [testability-is-governance-not-technical](skeletons-active/testability-is-governance-not-technical.md) ⭐⭐⭐⭐ **旗艦**：可測試性是治理問題不是技術問題（ID 要放在「擁有決策」的那層＝產品組裝層）。線十+三，**命中最弱的線十**
- [app-broken-is-misattribution-diagnosis-layers](skeletons-active/app-broken-is-misattribution-diagnosis-layers.md) ⭐⭐⭐⭐：「App 壞了」常是誣賴——後端/環境/GPU/覆蓋層診斷分層（3 真實案例）。線一 PDT+三
- [ai-finds-locator-human-defines-oracle](skeletons-active/ai-finds-locator-human-defines-oracle.md) ⭐⭐⭐⭐：AI 找 locator、人定 oracle 的誠實中場報告。線二+三。**也是未來 Test Corner talk 核心**

第二梯（strong / 偏技術實戰）：
- [theory-optimal-loses-to-org-reality](skeletons-active/theory-optimal-loses-to-org-reality.md) ⭐⭐⭐：理論最優敗給組織現實（官方 API vs 爬蟲）。線十+三
- [maintainability-is-ai-min-edit-unit](skeletons-active/maintainability-is-ai-min-edit-unit.md) ⭐⭐⭐：可維護性＝AI 要修改的最小單位有多小（三架構 AI ROI）。線三+二
- [flaky-is-information-not-randomness](skeletons-active/flaky-is-information-not-randomness.md) ⭐⭐⭐：flaky 是資訊不是隨機（社群頁閃退偵探）。線一 PDT
- [local-runs-ci-fails-environment-diff](skeletons-active/local-runs-ci-fails-environment-diff.md) ⭐⭐：本地能跑 CI 不能＝環境差異（CCT vs WebView）。線三

> 📌 **commit 挖到的額外新鮮素材**（尚未開骨架，之後可考慮）：blue 首登 onboarding 三層解、效能 243s→88s、CI 帳密參數化（ChipK 風格）、Compose recompose flaky（併入 flaky 那篇）。

> 🆕 **2026-07-17 再加 2 骨架**（做冒煙測試提案時長出）：
> - [ai-agent-lacks-industry-time-context](skeletons-active/ai-agent-lacks-industry-time-context.md) ⭐⭐⭐⭐：AI Agent 缺的不是能力、是「業界時間」累積的 context（知識 vs 時間換來的校準）。線二 + 難量化能力 moat，**招牌角度**、接 7/03「量錯的尺」與「AI 找 locator 人定 oracle」。
> - [test-independence-mobile-e2e](skeletons-active/test-independence-mobile-e2e.md) ⭐⭐⭐：單獨跑都過、合併跑就垮——手機 E2E 測試獨立性（狀態污染/順序依賴，7/17 兩 app 合併回歸真實踩坑）。線三/五、SEO 長尾強、鐵人賽 Day-2x 實戰。

### 其他長線題目（排隊中）

1. **PDT × 醫生問診類比**（補招牌主張深度，[analogy-series-proposals 主菜](skeletons-active/analogy-series-proposals.md)）——等線六冷卻 + 招牌系列再延伸時動
...
6. **PDT × QBT 雙層架構戰略**（**2026-06-01 新增**）：詳見 [pdt-vs-qbt-positioning.md](skeletons-done/pdt-vs-qbt-positioning.md)。
...
### 📌 已發文章裡開的坑（待回填）（2026-06-26 盤點）

> 掃描全部已發文章，找出「對讀者開了、但還沒寫」的承諾。回填這些 = 兌現對讀者的承諾（信任價值），跟一般長線題目性質不同。**UI 自動化系列（二~五）也是這類坑，但已完整記在上面那節、不重列。**

| 來源文章 | 開的坑（原句精神） | 待寫文章 | 優先 |
|---|---|---|---|
| problem-driven-testing-intro（招牌文） | 「在只看通過率的主管、只看時程的 PM、只看 bug 數的團隊裡，你要怎麼把 PDT 真的推動起來？」 | **PDT 在指標錯位組織裡怎麼落地** | ⭐⭐⭐ 最高曝光、料最齊（接 coach：忙=價值 / 別佈道 / 旺季淡季 / 接班人） |
| frontend-ui-vocabulary-for-qa ＋ proving-a-bug-is-gone（出現 2 次） | 「希望有天能測到給盲人 / 視障者使用的股票看盤 APP」 | **無障礙（A11y）測試實戰** | 🟡 等真的接到案子再寫（自承 PM 還沒開 ticket、硬寫違反不冒充經驗） |
| boredom-is-a-signal | Appium images plugin 4.1.1 長跑穩定性「還在觀察」 | Appium plugin 長跑崩潰踩坑筆記 | ⚪ niche、低優先、隨手記即可 |

> ⚠️ 另一個坑——ai-as-yes-man 結尾預告的「QA schema 篇」——**已由 [pass-fail-not-enough-for-ai-testing](/post/pass-fail-not-enough-for-ai-testing/) 填上（2026-06-18）**，不再列入。
