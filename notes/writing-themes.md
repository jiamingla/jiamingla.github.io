...
**最後更新**：2026-06-12（14 條主題線，6/12 新加線十四：品質之外 / 輕量職場人際與心理學）
**並進文件**：
- [writing-roadmap.md](writing-roadmap.md) — 進度快照、寫作節奏、發稿順序（操作型，常看）
- [tools-roadmap.md](tools-roadmap.md) — 工具優先策略

> 這份檔案是**長期主題版圖**——14 條主題線各自累積到哪、缺什麼。比 roadmap 較少更新，主要在「新題目該歸到哪條線、這條線該不該加題、整體版圖均衡嗎」這類**策略性問題**浮現時來查。
>
...
- 詳細提案見 [analogy-series-proposals.md](skeletons-active/analogy-series-proposals.md)
...
- 🆕 **[ai-test-output-default-reader-is-ai](../source/_drafts/ai-test-output-default-reader-is-ai.md)** — 為什麼 AI 給的測試案例讀起來特別累？因為它的預設讀者不是你。三層 mismatch（專業 / 語言 / 基礎假設）+ 四個讀 AI 輸出的具體技巧（為自己 prompt design / 詞彙考試 / 翻譯 / glossary）。**起源**：2026-05-14 寫 [frontend-ui-vocabulary](skeletons-done/frontend-ui-vocabulary-planning.md) 篇時衍生出來的更大命題。**讀者**：中文圈 QA + 單一專業背景——這是**繁體中文 QA 部落格的 differentiator**（英文圈不討論這個 angle）。**配對**：跟 [yes-man](../source/_drafts/ai-as-yes-man-rd-pm-trust-calibration.md) 形成「**Working with AI Output 雙篇**」（會問 + 會讀）
...
- 🆕 ⭐⭐ **〈一份測試 lib 服務 20 個 APP——元素定位的漸進式抽象策略〉**（5/28 從工作觀察整理）。**主論點**：跨產品線共用測試 lib 的元素定位設計——漸進式三階段（fallback chain → LocatorStrategy → per-APP class），「重複 3 次才抽象」「抽通用 pattern 不抽 APP」。**起源**：2026-05-28 cross-app-locator-strategy 工程備忘。**含公司指紋需洗**，但長尾力極強。**跟 pom-refactor 形成「POM 雙篇」**：pom-refactor 處理 single APP「能跑→可維護」、本篇處理「single APP → multi APP」規模化。詳見 [cross-app-locator-strategy.md](skeletons-done/cross-app-locator-strategy.md)
...
### 🗄️ 深度版（抽屜文，暫不發表）：[pm-bug-management-structural-DRAWER.md](skeletons-done/pm-bug-management-structural-DRAWER.md)
...
**草稿**：
- [cant-teach-while-doing.md](skeletons-active/cant-teach-while-doing.md)（生鮮觀察備忘，我懂了我爸做水電為什麼沒辦法邊教邊做）
- [how-to-start-low-pressure-book-club](../source/_drafts/how-to-start-low-pressure-book-club.md)（15 分鐘無負擔讀書會，抗完美主義實踐）
...
**為什麼這條線對目前狀況特別合適**：
- **直接解 [pdt-vs-qbt-positioning.md](skeletons-done/pdt-vs-qbt-positioning.md) 裡的 todo**：「PDT 主篇加一節『跟業界既有概念的關係』」——書評是這個 todo 最佳實踐手段
...
| 5 | 神經科學讀程式碼 (Ivanova 2020, Siegmund 2014) × QA 閱讀 | [ai-test-case-reading-bottleneck.md](skeletons-active/ai-test-case-reading-bottleneck.md) 已展開 | 稀有跨域題、英文圈也少見 |
...
## 線十四：品質之外（Beyond Quality） / 輕量職場人際與心理學（2026-06-12 新加）

**起源**：為了擴大部落格受眾，吸引對軟體測試完全沒有概念的人願意閱讀。用軟體工程或測試的邏輯/比喻，去解讀職場上的人際互動、溝通盲區與個人心理學，為非測試人員提供啟發。這也是為非技術的朋友（交友圈）量身打造的入口主題線。
...
- [successor-communication-and-ai-impatience.md](skeletons-active/successor-communication-and-ai-impatience.md) — **2026-06-10 新**：大人學接班人（溝通樞紐）的實踐、尋求平行肯定，以及 AI 即時回饋帶來的「組織耐性赤字」反思。（原屬線六，可與此線雙向對齊）
- 🆕 **〈人際關係的極限測試：我們如何定義彼此的邊界？〉**（邊界思維 / Boundary Thinking）
  - **核心**：借用軟體測試的「邊界值分析」與「壓力測試」，探討職場與日常人際關係的心理底線。如何在不破壞「系統（關係）」的前提下，優雅地摸索並界定自己與他人的邊界，避免發生系統崩潰（情緒失控）。
- 🆕 **〈日常溝通的重構指南：告別重複出現的 Bug 模式〉**（日常除錯 / Life Debugging）
  - **核心**：當生活中一再出現相同的衝突模式時，就像是程式碼裡的「壞味道（Code Smells）」。如何不帶情緒地對日常溝通進行重構？抓出那些無效且重複的「死循環（Dead Loop）」，透過微小的對話調整來「修補」關係。
- 🆕 **〈面對他人的黑箱：在看不見源碼的情況下如何順暢對話？〉**（黑箱人生 / Black-Box Life）
  - **核心**：在人際交往中，我們無法直視他人的靈魂深處，這是一場天然的「黑箱測試」。我們只能給予「輸入（話語、行動）」，並觀察「輸出（反應、情緒）」。如何接受不透明，並以健康、穩健的方法與這個黑箱系統安全共處。
- 🆕 **〈講對是不夠的：同理心溝通的「非功能性需求」〉**（人際除錯 / Non-Functional Requirements in Empathy）
  - **核心**：溝通不只要把「功能（事情）」講對，還要考慮「非功能性需求」（如語氣、時機、聆聽姿態——等同於系統的易用性與效能）。分析為什麼很多時候「邏輯完全正確」的論述，在人際溝通中卻會直接導致對手主機（聽者大腦）拒絕連線。

**狀態**：新開拓的非技術輕量線，是擴大部落格受眾（General Readers）與提升可讀性的核心支柱。
