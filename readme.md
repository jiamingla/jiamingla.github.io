...
這是一個基於 [Hexo](https://hexo.io/) 的靜態部落格專案。

[https://jiamingla.github.io/](https://jiamingla.github.io/)

## 分支管理策略

在一個典型的 Hexo 專案中，`main` 分支與 `gh-pages` 分支扮演的角色完全不同：

- **main**: 儲存原始碼與設定檔（你的生財工具）。
- **gh-pages**: 儲存生成的靜態網頁檔案（實際給瀏覽器看的內容）。

### 1. main 分支 (原始碼分支)

這是你進行寫作、修改主題、調整設定的地方。主要包含：
...
- `2024/01/xx/post-name/`：根據你的 permalink 設定生成的文章 HTML 頁面。
...
1.  你在 `main` 的 `source/` 下寫 Markdown。
2.  執行 `hexo g` 後，Hexo 會將其轉換為靜態 HTML 放到 `public/`。
3.  執行 `hexo d` 後，Hexo 會將 `public/` 的內容推送（Push）到 `gh-pages` 分支，完成發布。
