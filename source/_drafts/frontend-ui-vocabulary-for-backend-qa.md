---
urlname: frontend-ui-vocabulary-for-backend-qa
title: 後端 QA 該認識的 31 個前端 UI 詞彙——從 AI 給我 skeleton loading 那天開始補課
date: 2026-10-15 22:00:00
tags: [軟體測試, 前端測試, UI/UX, QA 入門, 後端轉前端, 跨領域學習, AI 測試, 品質工程]
---

> ✏️ 這是一篇種子稿。起源：2026-05-14 AI 給我的測試案例裡寫了 `skeleton loading`，我意識到自己是專精後端測試的 QA，對前端顯示效果 / 專有名詞的知識體系其實有缺口。

---

## 預設讀者（2026-05-14 確定）

**主要讀者**：背景以後端為主、開始要碰前端測試的 QA——可能是：
- 過去測 API、DB、business logic 的人
- 工作環境慢慢往全端測試擴張
- 開始用 AI 寫測試案例，發現 AI 用的詞彙自己不熟

**次要讀者**：新進 QA / 跨領域轉 QA 的工程師——也讀得進去。

**SEO 角度**：「skeleton loading 是什麼」「toast notification」「frontend ui terms」這種搜尋是穩定長尾。
...
那一刻我意識到——**我從來沒問過自己這個專業詞彙缺口**。我是專精後端測試的 QA，前端 UI 的整個詞彙體系（loading 狀態、用戶反饋、表單狀態、動畫術語...）我都是憑直覺看，沒有系統性學過。
...
於是這篇文章想做的事——**幫像我一樣的後端背景 QA，把前端 UI 詞彙的最小生存集合補起來**。
...
- **兩套詞彙幾乎不重疊**——後端 QA 自然很少接觸到
- 過去 RD 分工清楚時，QA 也跟著只測自己熟的部分

但現在三件事在改變：
1. **AI 給的測試 prompt 跨越前後端詞彙**——AI 不知道你 specialize 在哪邊
2. **全端開發越來越普遍**——QA 也被期待跟上
