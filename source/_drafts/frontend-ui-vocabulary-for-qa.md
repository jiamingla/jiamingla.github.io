...
那一刻我意識到——**我從來沒問過自己這個專業詞彙缺口**。前端 UI 的整個詞彙體系（loading 狀態、用戶反饋、表單狀態、動畫術語...）我都是憑直覺看，沒有系統性學過。我猜這不是只有我——**任何沒刻意盤點過前端 UI 詞彙的 QA，可能都有類似缺口**。
...
| **CTA（Call to Action）** | 「呼籲行動」按鈕——畫面上希望使用者點的主要動作（「立即註冊」「升級」「開始試用」） | **視覺優先級明確**（主 CTA / 次 CTA 對比清楚）、**文字具體**（「立即註冊」優於「OK」）、loading / disabled 狀態完整 |
