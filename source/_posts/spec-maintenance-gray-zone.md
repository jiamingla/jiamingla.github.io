...
| 維度 | 傳統 QA | SDET |
|---|---|---|
| 主要產出 | 測試案例、<br>bug report、<br>品質報告 | 測試框架、<br>自動化基礎設施、<br>把測試寫成 code |
| 在開發流程的位置 | 下游驗證者 | 上下游同時參與，<br>常常進設計會 |
| 跟 spec 的關係 | 讀 spec 來測 | 寫 spec、改 spec、<br>把 spec 變成<br>可執行的測試 |
| 程式能力預期 | 看得懂、會用工具 | 等同 RD，<br>能寫 production-grade<br>程式碼 |
| 「定義 + 驗收」衝突 | 高<br>（因此原則上不該扛<br>spec 維護） | 低<br>（角色設計上就同時在兩端，<br>是組織明知並接受的<br>trade-off） |
