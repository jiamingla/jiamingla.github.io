...
title: 「無聊」是 QA 的診斷工具：判斷一個手動測試該不該被自動化的五個訊號
date: 2026-05-06 18:45:00
tags: [軟體測試, 自動化測試, 測試自動化, 篩選功能, 品質工程, QA當責, ROI, 問題驅動測試, 無聊, 煩, 無聊心理學]
---
...
到了第二天下午，我手指麻了，眼睛酸了，開始打嗑睡，但更難受的是另一個東西——
...
書裡的核心定義很精準——**無聊不是「沒事做」，是「心智空閒」加上「想投入某件事卻找不到那件事」的組合**。一個人不會因為閒而無聊，會因為**想做點什麼但現在做的事沒用到自己**而無聊。

更微妙的是，書裡有一個我很久沒想清楚的觀察：**「缺乏投入」比「缺乏意義」更容易直接引起無聊**。所以滑社群媒體雖然意義稀薄，但因為提供投入感，可以**暫時**把無聊壓下去。
...
把這份呼喊聽進去，比任何 ROI 計算都更有重量。把篩選自動化掉這件事，意義不只是「我輕鬆一點」——它是**把人的能力解放回社會真正需要由人做的地方**。

這跟前面五節的論述其實是同一件事的不同層次：操作上是 ROI、組織上是分工、職涯上是視野——而**存在意義上**，它是無聊作為一種訊號，呼召你投入真正需要你的事。
...
## 八、如果現況真的動不了：給結構性受困的 QA 一條 plan B

前面七節有一個隱含前提：**你的工作環境給得起你動手改變的空間**。

但我自己上一份工作就在另一種情境裡：

- 系統老到沒有測試 seam，連 mock 都掛不上
- 系統有上雲架構但測試工具沒跟上——什麼時候該打 DB、什麼時候該抓物件儲存、跨服務怎麼模擬，每一題都要花時間想清楚
- 微服務拆得很細，光是搞清楚一條使用者流程跨幾個服務、每個服務該不該驗，就先吃掉一半時間
- 測試環境建置需要跨團隊協作，QA 一半時間花在組織層面、不在技術層面

第七節那三步——量化、估投資、提案——你都做了，主管也認，但**結構上就是動不了**。這節寫給卡在這種情境的 QA。

**動不了的時候，你還能守住三件事**。

### 1. 機械跟判斷的時間分界

不能把機械部分自動化掉，至少**時間上隔離**——

- 機械操作集中在注意力最低的時段（例如剛上班的前 2 小時，腦袋還沒熱起來）
- 判斷部分留給精神最好的時段，做探索式測試、追不和諧感、寫 bug report
- **不要邊機械邊判斷**——這個切換成本會吃掉你最寶貴的腦力

簡單的工具能幫上忙：截圖工具減少手寫筆記、test data 模板化、checklist 結構化讓跑得更快。**這不是真的自動化，是把人工模式優化得更接近自動化**。

### 2. 把每次「煩」記成證據

你現在動不了，不代表組織永遠動不了。但你需要**累積證據**——

- 每個 sprint 記錄：「這部分如果有自動化會省 X 小時、目前佔我 Y% 工時」
- 哪些 bug 是在「機械模式時段」漏掉的（這是強證據——證明人工跑會出包）
- 重大線上事件事後 review 時，能不能指出「這個如果有 regression test 就會被擋下」

組織會動的時機通常不是「主動規劃」，是「**某次出事之後**」。那時候有沒有人手上握著一份完整的成本紀錄，會決定資源往哪邊跑。你今天跑的每一個機械測試，都是在累積那份紀錄。

### 3. 守住識別力本身

這是最重要的一條。

當你長期卡在「煩 / 無聊」的環境裡，最大的風險不是技術沒進步，是**你開始懷疑自己**——是不是我不夠敬業？是不是別人都能撐我撐不下去？

接回第六節《無聊心理學》那個論點：**你身上的能動性在替整個系統呼喊「我被擺錯地方了」**。聽見這個呼喊、辨認出它、把它寫下來——**這份識別力本身就有價值，即使結構上你動不了**。

某天你會跳槽、會被推薦、會升到有授權改變的位置。**那時候，今天這份識別力會變成你的判斷資產**——你會記得哪些「煩」是組織該解決的、哪些是任務本身就該扛的。你不會把「QA 就是要忍」當作職業徽章。

---

> **守住識別力，比逼自己出產出更難、也更重要**。

當結構動不了的時候，能不能不被它磨掉判斷力——這本身就是 QA 紀律的一部分。

---

## 九、結語：把「煩」當作 QA 的診斷儀器
